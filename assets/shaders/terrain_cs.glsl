...
{
    return (((z * (u_height + 1)) + y) * (u_width + 1)) + x;
}
...
void main()
{
	...
	if (id.x > u_width || id.y > u_height || id.z > u_depth)
		return;
	...
	point.y = id.y;
	point.z = id.z;
	point.w = id.z - (float(u_depth) * 0.5);
	vec3 n = vec3(float(id.x), float(id.y), float(id.z)) / 16.0;
	float feature = float(u_depth) * 0.2;
	...
	*/
	...
	vec3 pos = vec3(float(id.x), float(id.y), float(id.z));
	vec3 center = vec3(0.0);
	vec3 boundsSize = vec3(float(u_width), float(u_height), float(u_depth));
    pos = center + ((pos - (boundsSize * 0.5)) * spacing);
    ...
    for (int j = 0; j < octaves; j++) {
		vec3 samplePos = pos;
		samplePos.x += snoise(pos) * 0.2;
		samplePos.y += snoise(pos * 2.0) * 0.2;
    ...
    float finalVal = -(pos.z + floorOffset) + noise * noiseWeight;// + (pos.y % params.x) * params.y;
	...
	//finalVal = pos.z - 8;
	points[indexFromCoord(id.x, id.y, id.z)] = vec4(pos, -finalVal);
}
